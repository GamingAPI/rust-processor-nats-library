...
/**
 * Module which wraps functionality for the `rust/servers/{server_id}/players/{steam_id}/events/reported` channel
 * @module rustServersServerIdPlayersSteamIdEventsReported
 */
/**
 * Internal functionality to setup subscription on the `rust/servers/{server_id}/players/{steam_id}/events/reported` channel 
 * 
...
export function subscribe(
  onDataCallback: (
    ...
    msg ? : ServerPlayerReported, server_id ? : string, steam_id ? : string) => void,
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      let subscription = await client.subscribe(`rust.servers.${server_id}.players.${steam_id}.events.reported`, (err, msg) => {
        if (err) {
          onDataCallback(NatsTypescriptTemplateError.errorForCode(ErrorCode.INTERNAL_NATS_TS_ERROR, err));
        } else {
          const unmodifiedChannel = `rust.servers.{server_id}.players.{steam_id}.events.reported`;
          ...
          onDataCallback(undefined, ServerPlayerReported.unmarshal(receivedData), serverIdParam, steamIdParam);
        }
      }, subscribeOptions);
