...
/**
 * Module which wraps functionality for the `rust/servers/{server_id}/players/{steam_id}/events/banned` channel
 * @module rustServersServerIdPlayersSteamIdEventsBanned
 */
/**
 * Internal functionality to setup subscription on the `rust/servers/{server_id}/players/{steam_id}/events/banned` channel 
 * 
...
export function subscribe(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      let subscription = await client.subscribe(`rust.servers.${server_id}.players.${steam_id}.events.banned`, (err, msg) => {
        if (err) {
          onDataCallback(NatsTypescriptTemplateError.errorForCode(ErrorCode.INTERNAL_NATS_TS_ERROR, err));
        } else {
          const unmodifiedChannel = `rust.servers.{server_id}.players.{steam_id}.events.banned`;
