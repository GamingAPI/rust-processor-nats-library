...
/**
 * Module which wraps functionality for the `rust/servers/{server_id}/players/{steam_id}/events/items/{item_id}/pickup` channel
 * @module rustServersServerIdPlayersSteamIdEventsItemsItemIdPickup
 */
/**
 * Internal functionality to setup subscription on the `rust/servers/{server_id}/players/{steam_id}/events/items/{item_id}/pickup` channel 
 * 
...
export function subscribe(
  onDataCallback: (
    ...
    msg ? : ServerPlayerItemPickup, server_id ? : string, steam_id ? : string, item_id ? : string) => void,
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      let subscription = await client.subscribe(`rust.servers.${server_id}.players.${steam_id}.events.items.${item_id}.pickup`, (err, msg) => {
        if (err) {
          onDataCallback(NatsTypescriptTemplateError.errorForCode(ErrorCode.INTERNAL_NATS_TS_ERROR, err));
        } else {
          const unmodifiedChannel = `rust.servers.{server_id}.players.{steam_id}.events.items.{item_id}.pickup`;
          ...
          onDataCallback(undefined, ServerPlayerItemPickup.unmarshal(receivedData), serverIdParam, steamIdParam, itemIdParam);
        }
      }, subscribeOptions);
