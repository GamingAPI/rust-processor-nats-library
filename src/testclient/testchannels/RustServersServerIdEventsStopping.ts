 ...
 */
/**
 * Internal functionality to setup subscription on the `rust/servers/{server_id}/events/stopping` channel 
 * 
...
export function subscribe(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      let subscription = await client.subscribe(`rust.servers.${server_id}.events.stopping`, (err, msg) => {
        if (err) {
          onDataCallback(NatsTypescriptTemplateError.errorForCode(ErrorCode.INTERNAL_NATS_TS_ERROR, err));
        } else {
          const unmodifiedChannel = `rust.servers.{server_id}.events.stopping`;
