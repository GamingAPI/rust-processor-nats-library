...
/**
 * Module which wraps functionality for the `rust/servers/{server_id}/players/{steam_id}/events/unbanned` channel
 * @module rustServersServerIdPlayersSteamIdEventsUnbanned
 */
/**
 * Internal functionality to setup subscription on the `rust/servers/{server_id}/players/{steam_id}/events/unbanned` channel 
 * 
...
export function subscribe(
  onDataCallback: (
    ...
    msg ? : ServerPlayerUnbanned, server_id ? : string, steam_id ? : string) => void,
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      let subscription = await client.subscribe(`rust.servers.${server_id}.players.${steam_id}.events.unbanned`, (err, msg) => {
        if (err) {
          onDataCallback(NatsTypescriptTemplateError.errorForCode(ErrorCode.INTERNAL_NATS_TS_ERROR, err));
        } else {
          const unmodifiedChannel = `rust.servers.{server_id}.players.{steam_id}.events.unbanned`;
          ...
          onDataCallback(undefined, ServerPlayerUnbanned.unmarshal(receivedData), serverIdParam, steamIdParam);
        }
      }, subscribeOptions);
