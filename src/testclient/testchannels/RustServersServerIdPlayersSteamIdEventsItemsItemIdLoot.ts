 ...
 */
/**
 * Internal functionality to setup subscription on the `rust/servers/{server_id}/players/{steam_id}/events/items/{item_id}/loot` channel 
 * 
...
export function subscribe(
  onDataCallback: (
    ...
    msg ? : ServerPlayerItemLoot, server_id ? : string, steam_id ? : string, item_id ? : string) => void,
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      let subscription = await client.subscribe(`rust.servers.${server_id}.players.${steam_id}.events.items.${item_id}.loot`, (err, msg) => {
        if (err) {
          onDataCallback(NatsTypescriptTemplateError.errorForCode(ErrorCode.INTERNAL_NATS_TS_ERROR, err));
        } else {
          const unmodifiedChannel = `rust.servers.{server_id}.players.{steam_id}.events.items.{item_id}.loot`;
          ...
          onDataCallback(undefined, ServerPlayerItemLoot.unmarshal(receivedData), serverIdParam, steamIdParam, itemIdParam);
        }
      }, subscribeOptions);
