...
/**
 * Module which wraps functionality for the `rust/servers/{server_id}/events/command` channel
 * @module rustServersServerIdEventsCommand
 */
/**
 * Internal functionality to setup subscription on the `rust/servers/{server_id}/events/command` channel 
 * 
...
export function subscribe(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      let subscription = await client.subscribe(`rust.servers.${server_id}.events.command`, (err, msg) => {
        if (err) {
          onDataCallback(NatsTypescriptTemplateError.errorForCode(ErrorCode.INTERNAL_NATS_TS_ERROR, err));
        } else {
          const unmodifiedChannel = `rust.servers.{server_id}.events.command`;
