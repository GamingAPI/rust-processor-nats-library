 ...
 */
/**
 * Internal functionality to setup subscription on the `v0/rust/servers/{server_id}/players/{steam_id}/events/disconnected` channel 
 * 
...
export function subscribe(
  onDataCallback: (
    ...
    msg ? : ServerPlayerDisconnected, server_id ? : string, steam_id ? : string) => void,
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      let subscription = await client.subscribe(`v0.rust.servers.${server_id}.players.${steam_id}.events.disconnected`, (err, msg) => {
        if (err) {
          onDataCallback(NatsTypescriptTemplateError.errorForCode(ErrorCode.INTERNAL_NATS_TS_ERROR, err));
        } else {
          const unmodifiedChannel = `v0.rust.servers.{server_id}.players.{steam_id}.events.disconnected`;
          ...
          onDataCallback(undefined, ServerPlayerDisconnected.unmarshal(receivedData), serverIdParam, steamIdParam);
        }
      }, subscribeOptions);
