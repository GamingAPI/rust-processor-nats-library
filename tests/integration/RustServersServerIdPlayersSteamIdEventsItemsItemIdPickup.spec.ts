...
describe('rust/servers/{server_id}/players/{steam_id}/events/items/{item_id}/pickup can talk to itself', () => {
  ...
  it('can send message', async () => {
    ...
    const subscription = await testClient.subscribeToRustServersServerIdPlayersSteamIdEventsItemsItemIdPickup((err, msg, server_id, steam_id, item_id) => {
        receivedError = err;
        receivedMsg = msg;
    ...
    await client.publishToRustServersServerIdPlayersSteamIdEventsItemsItemIdPickup(publishMessage, ServerIdToSend, SteamIdToSend, ItemIdToSend);
